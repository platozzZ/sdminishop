<template>
	<div class="order_ii" :style="{'overflow':overflow}">
		...
		<div ref='box' id="boxs" class='relative' @touchmove="setHead">
			...
			<!-- #ifndef H5 -->
			<!--  -->
			<div v-if="!bargain" class="gd_header" :class="{gd_headerplus:headerplus}" :style="{top: customBar + 'px'}">
				<view class="detail-homeImg" v-if="option.fromfb == '1'" @tap="_goback()">
					<text class="cuIcon-home"></text>
				</view>
				<img :src="bback_img" class="gd_back" @tap="_goback()" v-else>
			...
			<div v-if="!bargain" class="gd_header" :class="{gd_headerplus:headerplus}" >
				<img v-if="isDistribution==false&&!bargain&&active!=6" :src="gw_img" class="gd_share" @tap="_gocart()" />
				<div class="cartnum" v-if='isDistribution==false&&allCartNum&&active!=6'>{{allCartNum}}</div>
			</div>
			<!-- #endif -->
			...
			<template v-if="loadFlag">
				...
				<div class="relative">
					<swiper class='swiper' @change="swiperChange">
						<swiper-item v-for='img in pro.img_arr' :key='img.id'>
							<image class="swipe" :src="img"></image>
						</swiper-item>
					</swiper>
				...
				<div class='goods_name' v-if='!bargain'>
					...
					<!-- <button type="primary" plain="true" @click="mobShareWebpage()">点击分享链接</button> -->
					<p class='goods_proName'>{{pro.name}}</p>
					<div class="volume">
						<span>运费金额：{{pro.yunfei}}</span>
				...
				<view 
					class='p-detail-coupon p-flex p-align-center p-justify-between' 
					@tap="getCoupon()" 
					v-if="isDistribution==false && !bargain&& active != 6 && coupon_status==true"
				>
					<view class="p-detail-title">优惠券</view>
					<view class="p-detail-content">
						<view v-for="(item,index) in coupon_str" :key="index" class="p-coupon_data" :style="'background-image:url(' + coupon_img + ')'">{{item}}</view>
					</view>
					<image class='p-arrow' :src="you_img"></image>
				</view>
				<div class='hr'></div>
				<!-- 非砍价商品选择商品规格 -->
				...
				<view v-if='!bargain' class='p-guige p-flex p-align-center p-justify-between' @tap="_shopping1()" ref="homeHead">
					<view>{{haveSkuBean==""?"选择商品规格":haveSkuBean.name}}</view>
					<image class='p-arrow' :src="you_img"></image>
				</view>
		...
		<view 
			class="p-mask_d"
			:class='mask_display1?"goodAnimate1":mask_display?"goodAnimate": ""' 
			v-if='mask_display' 
		>
			<view class="p-mask_guige">
				<view class="p-mask_one p-flex p-align-end p-justify-start" @touchmove.stop.prevent>
					<view class='p-mask_imgDiv'>
						<image v-if='loadImg' @load='_loadImg()' class="shangp" :src="imgurl"></image>
						<image v-if='!loadImg' :src="load_img" class='img'></image>
					</view>
					<view class='p-mask_pric p-flex p-flex-direction '>
						<view class='p-mask_price'>￥<text class='font_30'>{{price}}</text></view>
						<view class='p-mask_pric_num'>库存{{num}}</view>
					</view>
					<image class="p-cha" :src="guan_img" @tap="_mask_f()"></image>
				</view>
				
				<view class="p-mask_over p-flex p-flex-direction">
					<view class='p-mask_two' v-for='(item,indx) in attrList' :key='indx'>
						<view>{{item.attrName}}</view>
						<view class="p-flex p-align-center p-justify-start p-list">
							<view 
								class="p-flex p-align-center p-justify-center"
								v-for='(items,index) in item.attr' 
								:key='index' 
								:class="items.enable ? (items.select ? 'p-orange':'p-select') : 'back'"
					...
					<view class='p-mask_num p-flex p-align-center p-justify-between'>
						<view>数量</view>
						<view class='p-goods_mun p-flex p-align-center p-justify-between'>
							<view class='p-goods_mun_span p-flex p-align-center p-justify-center' @tap="_reduce">
								<image :src="numb==1?jian_hui:jian_hei"></image>
							</view>
							<view class='p-mun p-flex p-align-center p-justify-center'>{{ numb }}</view>
							<view class='p-goods_mun_add p-flex p-align-center p-justify-center' @tap="_add">
								<image :src='numb<num?jia_hei:jia_hui'></image>
							</view>
						</view>
					</view>
				</view>
			...
			<view class='p-mask_querenDiv'>
				<view class='p-mask_quren' @tap="_confirm">确认</view>
			</view>
		</view>
		<!--  遮罩 -->
		<!-- <div class='mask' @touchmove.stop.prevent v-if='mask_display' :style="{'bottom':(isiPhoneX? '34px':'0'),'paddingBottom': (isiPhoneX? '34px':'0')}"></div>
		<div class="mask_d" :class='mask_display1?"goodAnimate1":mask_display?"goodAnimate": ""' v-if='mask_display' :style="{'marginBottom': (isiPhoneX? '34px':'0')}">
			...
			<div class='mask_querenDiv'>
				<div class='mask_quren' :style="{'bottom': (isiPhoneX? '34px':'0')}" @tap="_confirm">确认</div>
			</div>
		</div> -->
...
<script>
	...
	export default {
		...
		onLoad(option) {
			...
			let isiPhoneX = uni.getStorageSync('isiPhoneX')
			if(isiPhoneX){
				me.isiPhoneX = isiPhoneX
			}
			if (me.leftTime) {
			...
			// console.log("uni.getStorageSync('access_id'):" ,uni.getStorageSync('access_id'))
			// console.log('me.$store.state.access_id:' ,me.$store.state.access_id)
			// me.access_id = uni.getStorageSync('access_id') ? uni.getStorageSync('access_id') : me.$store.state.access_id
			me.hasorder = option.hasorder;
			// console.log('me.access_id:',me.access_id)
		...
		methods: {
			// mobShareWebpage() {
			// 	console.log(pluginMobShare);
			// 	pluginMobShare.generalShare({
			// 		platName: SSDKPlatformID.WeChat,
			// 		mobAppkey: '30032a109847a',
			// 		mobSecret: '7b81f381ed03ba150aea3f4c68b5e27e',
			// 		text: 'text',
			// 		title: "title",
			...
			goPay() {
				...
				console.log('bargain_id:',bargain_id);
			...
			_reduce() {
				...
				console.log(this.numb < this.num);
			...
			_buy() {
				var me = this;
				// alert('me.axios_complete:' + me.axios_complete)
				if (!me.axios_complete) {
					return false
				}
				if (me.status == 2) {
					
					// alert('me.pro.canbuy:' + me.pro.canbuy)
					if (me.pro.canbuy == 1) {
...
<style lang="less" scoped>
	...
	.p-mask_d{
		...
		height: calc(800upx + constant(safe-area-inset-bottom) / 2);
		height: calc(800upx + env(safe-area-inset-bottom) / 2);
	...
	.p-mask_two, .p-mask_num {
	    padding: 36upx 30upx;
	    border-bottom: 1px solid #eee;
		color: #999999;
		font-size: 28upx;
	}
	.p-mask_two .p-list>view{
		min-width: 60upx;
		border: 1px solid #eee;
	...
	.p-detail-coupon .p-detail-title{
		// margin-right: 37px;
	}
	.p-detail-coupon .p-detail-content{
		padding: 0 30upx;
		display: grid;
		align-items: center;
		flex: 1;
	...
	.p-coupon_data{
		...
		min-width: 133rpx;height: 32rpx;
		padding: 0 10rpx;
		box-sizing: border-box;
		font-size: 22rpx;line-height: 32rpx;
		text-align: center;color: #FF3B3B;
	...
	.goods_foot{
		height: calc(98upx + constant(safe-area-inset-bottom) / 2);
