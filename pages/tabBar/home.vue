...
<script>
	...
	export default {
		...
		created() {
			window.loginResult = this.loginResult;
		},
		onLoad(option) {
			console.log(option);
			console.log('$jweixin:',this.$jweixin)
			console.log('$uni:',this.$uni)
			var me = this;
			me.isWeixinBrowser()
			// 有分享的属性
...
.cu-custom .cu-bar .bar-action{
	font-size: 38rpx;
}
.cu-custom .cu-bar .rightAction .bar-action{
	font-size: 36rpx;
}
