	...
	<div ref='cart' class="">
		...
		<div v-else style="position: relative;">
			...
			<!-- <div class='p-page_title'>
				<div class="p-header" :style="{top:halfWidth}">
					<text class="cuIcon-homefill bar-action" @tap="closeWebview"></text>
					<span class='p-span'>购物车</span>
					<div class="p-manage" v-if="goods.length" @tap="_manage">{{manage_text}}</div>
				</div>
			</div> -->
			<!-- #endif -->
			<!-- #ifdef MP -->
			...
			<div class="bottom" v-if="goods.length&&!load">
			...
			closeWebview(){
				...
				if(ua.match(/MicroMessenger/i)=="micromessenger") {
					...
					this.$jweixin.miniProgram.getEnv((res)=>{
					   if (res.miniprogram) {
						   console.log("在小程序里");
							this.$jweixin.miniProgram.reLaunch({
								url: '/pages/homepage/homepage',
							}) 
					   } else {
						   this.$uni.webView.reLaunch({
								url: '/pages/homepage/homepage',
						   })
						   console.log("不在小程序里");
					   }
...
.bottom{
	bottom: calc(101upx + constant(safe-area-inset-bottom) / 2);
	bottom: calc(101upx + env(safe-area-inset-bottom) / 2);
}
.allgoods{
...
.cu-custom .cu-bar .action .bar-action{
	font-size: 36rpx;
	padding-right: 30rpx;
}
.cu-custom .cu-bar .rightManage{
	font-size: 30rpx;
	min-width: 100rpx;
	text-align: right;
}
