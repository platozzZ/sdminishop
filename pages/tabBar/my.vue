<template>
	<div class='order_ii' style='background-color: #F8F8F8;'>
		...
		<view class="p-order">
			<view class='p-flex p-align-center p-justify-between p-order-t'>
				<view>我的订单</view>
				<view class='p-flex p-align-center p-justify-end' @tap="_order(0)">
					<text class=''>查看全部</text>
					<image class='' :src="jiantou"></image>
				</view>
			</view>
			<view class="p-flex p-align-center p-justify-between p-order-b">
				<view class=" p-item p-flex p-flex-direction p-align-center p-justify-center" @tap="_order(1)">
					<view class="p-flex p-flex-direction p-align-center p-justify-between p-item-t">
						<image class='' :src="qianbao"></image>
						<view class='p-item-tags p-flex p-align-center p-justify-center' v-if='access_id1&&dfk_num'><text>{{dfk_num}}</text></view>
					</view>
					<view>待付款</view>
				</view>
				<view class=" p-item p-flex p-flex-direction p-align-center p-justify-center" @tap="_order(2)">
					<view class=" p-flex p-flex-direction p-align-center p-justify-between p-item-t">
						<image class='' :src="zu"></image>
						<!-- <view class='order_b' v-if='access_id1&&dfh_num'>{{dfh_num}}</view> -->
						<view class='p-item-tags p-flex p-align-center p-justify-center' v-if='access_id1&&dfh_num'><text>{{dfh_num}}</text></view>
					</view>
					<view>待发货</view>
				</view>
				<view class=" p-item p-flex p-flex-direction p-align-center p-justify-center" @tap="_order(3)">
					<view class=" p-flex p-flex-direction p-align-center p-justify-between p-item-t">
						<image class='' :src='dizhi'></image>
						<!-- <view class='order_b' v-if='access_id1&&dsh_num'>{{dsh_num}}</view> -->
						<view class='p-item-tags p-flex p-align-center p-justify-center' v-if='access_id1&&dsh_num'><text>{{dsh_num}}</text></view>
					</view>
					<view>待收货</view>
				</view>
				<view class=" p-item p-flex p-flex-direction p-align-center p-justify-center" @tap="_order(4) ">
					<view class=" p-flex p-flex-direction p-align-center p-justify-between p-item-t">
						<image class='' :src="messageImg"></image>
						<!-- <view class='order_b' v-if='access_id1&&dpj_num'>{{dpj_num}}</view> -->
						<view class='p-item-tags p-flex p-align-center p-justify-center' v-if='access_id1&&dpj_num'><text>{{dpj_num}}</text></view>
					</view>
					<view>待评价</view>
				</view>
				<view class=" p-item p-flex p-flex-direction p-align-center p-justify-center" @tap="_navigateTo1('../../pagesA/afterSale/afterSale')">
					<view class=" p-flex p-flex-direction p-align-center p-justify-between p-item-t" style="width: 80upx;margin: 0 auto;">
						<image class='' :src="yueImg"></image>
						<!-- <view class='order_b' v-if='access_id1&&th_num'>{{th_num}}</view> -->
						<view class='p-item-tags p-flex p-align-center p-justify-center' v-if='access_id1&&th_num'><text>{{th_num}}</text></view>
					</view>
					<view>退款/售后</view>
				</view>
			</view>
		...
		<!-- <view class='order'>
			<view class='order_one'>
				<view>我的订单</view>
				<view class='center' @tap="_order(0)">
					<text class='span'>查看全部</text>
					<image class='img' :src="jiantou"></image>
				</view>
			</view>
			<view class='order_two'>
				<view @tap="_order(1)">
					<view class="position_quan">
						<image class='img' :src="qianbao"></image>
						<view class='order_b' v-if='access_id1&&dfk_num'>{{dfk_num}}</view>
					</view>
					<view>待付款</view>
				</view>
				<view @tap="_order(2)">
					<view class="position_quan">
						<image class='img' :src="zu"></image>
						<view class='order_b' v-if='access_id1&&dfh_num'>{{dfh_num}}</view>
					</view>
					<view>待发货</view>
				</view>
				<view @tap="_order(3)">
					<view class="position_quan">
						<image class='img' :src='dizhi'></image>
						<view class='order_b' v-if='access_id1&&dsh_num'>{{dsh_num}}</view>
					</view>
					<view>待收货</view>
				</view>
				<view @tap="_order(4) ">
					<view class="position_quan">
						<image class='img' :src="messageImg"></image>
						<view class='order_b' v-if='access_id1&&dpj_num'>{{dpj_num}}</view>
					</view>
					<view>待评价</view>
				</view>
				<view @tap="_navigateTo1('../../pagesA/afterSale/afterSale')">
					<view class="position_quan" style="width: 80upx;margin: 0 auto;">
						<image class='img' :src="yueImg"></image>
						<view class='order_b' v-if='access_id1&&th_num'>{{th_num}}</view>
					</view>
					<view>退款/售后</view>
				</view>
	...
	export default {
		data() {
			return {
				...
				setShow: this.$store.state.channel!="zyjd",
		...
		onShow() {
			...
			console.log(this.customBar);
			this.access_id = this.$store.state.access_id ? this.$store.state.access_id : uni.getStorageSync('access_id')
		...
		methods: {
			...
			_axios() {
				var me = this
				var data = {
					access_id: me.access_id ? me.access_id : this.$store.state.access_id,
					module: 'app',
					action: 'user',
					app: 'index'
				}
