				...
				uni.request({
					...
					success:function(res){
						...
						if( data.list && data.list.length > 0 ){
							...
							if(me.fromUrl.indexOf("fromfb") != -1){
								let fromfb = me.getParam(me.fromUrl,'fromfb')
								let para = me.getUrlRelativePath(me.fromUrl,'para')
								console.log('fromfb:',fromfb);
								switch (fromfb) {
									case '1':
										uni.redirectTo({
											url: '../goods/goodsDetailed?' + para
										})
										break;
									case '2':
										uni.redirectTo({
											url: '../../pagesA/shop/coupon?' + para
										})
										break;
									case '3':
										uni.redirectTo({
											url: '../../pagesA/bargain/bargain?' + para
										})
										break;
									case '4':
										uni.switchTab({
...
                var that = this;
				if(that.time == 1 || type){
					if(that.fromUrl.indexOf("fromfb") != -1){
						// let para = that.getUrlRelativePath(that.fromUrl,'para')
						// console.log(para)
						// uni.redirectTo({
						// 	url: '../goods/goodsDetailed?' + para
						// })
