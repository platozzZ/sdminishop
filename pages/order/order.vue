<template>
	<div class='order'>
		...
		<div v-else style='position: relative;'>
			...
			<!-- <view class="p-detail">
				<view class="p-detail-name p-flex p-align-center p-justify-between" @click="_goStore(order.list[0].shop_id)">
					<view class="p-flex p-align-center">
						<image :src="storeImg" class="p-detail-shopname"></image>
						{{ order.list[0].shop_name }}
						<image :src="jiantou" class="p-detail-more"></image>
					</view>
				</view>
				<view class="p-detail-content p-flex p-justify-between" v-for="(item,index) in order.list" :key="'li'+index">
					<image :src='item.imgurl' class="p-avatar"></image>
					<view class="p-detail-content-c p-flex p-flex-direction p-justify-between" @tap="_goods(item.p_id)">
						<view class="p-c-title">{{item.p_name}}</view>
						<view class="p-c-size">{{item.size}}</view>
					</view>
					<view class="p-detail-action">
			...
			<ul class='order_goods'>
				<li  class="order_goods_lis" style="0 0 30upx 0">
					...
					<view class="p-detail-content p-flex p-justify-between" :key="'li'+index">
						<image :src='item.imgurl' class="p-avatar"></image>
						<view class="p-detail-content-c p-flex p-flex-direction p-justify-between" @tap="_goods(item.p_id)">
							<view class="p-c-title">{{item.p_name}}</view>
							<view class="p-c-size">{{item.size}}</view>
...
<style lang="less" scoped>
	...
	.p-detail-name image.p-detail-shopname{
		width: 33upx;
		height: 30upx;
		margin-right: 11upx;
	}
	.p-detail-name image.p-detail-more{
		width: 15upx;
		height: 27upx;
		margin-left: 16upx;
	}
