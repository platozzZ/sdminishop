<template>
	<div ref='cart' class="cart_f">
		...
		<div v-else class="order_detail">
			...
			<!--灰色间隔-->
			<div class="yh-line"></div>
			<view class="p-paycontainer" v-if="!isMini">
				<view class='order_coupon' v-if='!bargain'>
					<span class="yh-order_coupon-spanc">请选择支付方式</span>
					<!-- <span>￥{{needpay}}</span> -->
				</view>
				<view class='order_coupon' v-else>
					<span class="yh-order_coupon-spanc">请选择支付方式</span>
					<span>￥{{needpay.length != 0?needpay:price2}}</span>
				</view>
				<!-- 微信支付 -->
				<!-- #ifdef APP-PLUS || MP-WEIXIN || H5 -->
				<view class='p-pay' @tap='chooseWay("wxPay")' v-if="open_wxpay == true">
					<view class="p-yh-pay">
						<view class="p-pay_yue p-yh-pay-wx-d">
							<view class="p-yh-pay-s">
								<image class='p-pay_img' :src="pay_w" alt=""></image>
								<view class="p-yh-pay_div">
									<view class="p-yh-pay_div-p">
										微信支付
									</view>
								</view>
							</view>
							<img class='p-quan_img' :src="wxPayStatue?quan_hei:quan_hui" />
						</view>
					</view>
				</view>
				<!-- #endif -->
				...
				<view class='p-pay' @tap='chooseWay("aliPay")' v-if="open_alipay == true && !isWeixin">
					<view class="p-yh-pay">
						<view class="p-pay_yue p-yh-pay-wx-d">
							<view class="p-yh-pay-s">
								<image class='p-pay_img' :src="pay_z" alt=""></image>
								<view class="p-yh-pay_div">
									<view class="p-yh-pay_div-p">
										支付宝支付
									</view>
							...
							<img class='p-quan_img' :src="aliPayStatue?quan_hei:quan_hui" />
						</view>
					</view>
				</view>
				<!-- #endif -->
			...
			<!--支付方式-->
			<ul style='display: none;' v-if="false">
				<div>
					...
					<!-- 支付宝支付 -->
					<!-- #ifdef APP-PLUS || MP-ALIPAY || MP-TOUTIAO || H5 -->
					<li class='pay' @tap='chooseWay("aliPay")' v-if="open_alipay == true && !isWeixin">
						<div class="yh-pay">
							<div class="pay_yue yh-pay-wx-d">
								<div class="yh-pay-s">
			...
			<div class='foot' id='foot' :class='ishide==1?"hide":""'>
				...
				<!-- <p @tap="_pay_style()" v-if="!isMini">立即支付</p>
				<p @tap="weixinPay()" v-else>立即支付</p> -->
			</div>
		</div>
...
<script>
	...
	export default {
		...
		methods: {
			...
			_uni_redirectTo(url){
				// alert('orderDetailsr-_uni_redirectTo:' + url)
				uni.redirectTo({
					url
				})
			},
			...
			toMiniPay(e){
				console.log('toMiniPay:',e);
				this.$jweixin.miniProgram.navigateTo({
					url: '/pages/minipay/minipay?data=' + e
				})
				// wx.webView.navigateTo({
				// 	url: '/pages/minipay/minipay?data=' + e
				// })
			},
			/**
