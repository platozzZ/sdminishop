<script>
	...
	function weixin_minipay(me) {
		...
		// 	success: (e) => {
		var data;
		console.log('me.pay_name:',me.pay_name);
		console.log('me.bargain:',me.bargain);
		if (me.pay_name == "jp") {
			data = {
				// code: e.code,
		...
		uni.request({
			...
			data,
			success: (res) => {
				console.log('weixin_minipay-res:',res);
				uni.hideLoading();
				if (res.statusCode == 200) {
					let paymentData = res.data;
					console.log('weixin_minipay:',paymentData);
					me.toMiniPay(encodeURIComponent(JSON.stringify(paymentData)))
					// uni.requestPayment({
					// 	provider: 'wxpay',
					// 	timeStamp: paymentData.timeStamp,
					// 	nonceStr: paymentData.nonceStr,
