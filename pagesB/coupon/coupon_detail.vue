		...
		<div  class="order_detail" v-else>
				...
				<li class='goods'>
					...
					<div class='goods_right'>
						...
						<div class='goods_bottom'>
							<p><span class='font_22'>￥</span>{{jp_total}}元</p>
			...
			<li class='order_coupon'>
				...
				<span class='bold font_30'>
					合计：<font class='bold font_30 color_FF0'>￥{{price2}}</font>
				</span>
			</li>
			<!--支付方式-->
			<ul style=''>
				<li class='pay' v-if='price2>0'>
					<div class='w_100'>
						...
						<div v-if="useWallte" class='deco'>
							<div class='deco_title'>使用余额</div>
							<div class='deco_data'>
								<input class='ipt' :value="value" type='digit' @focus="_hide()" @blur="_show()" @input="replaceInput" 
								 placeholder="请输入抵扣金额">
							</div>
						</div>
						<!-- 余额支付结束 -->
			...
			<div class='foot' id='foot' :class='ishide==1?"hide":""'>
				<div>
					<p class='foot_price'>付款金额：￥{{price2}}</p>
				</div>
				<p @tap="_pay_style()">{{price2>0?'立即支付':'立即兑换'}}</p>
			</div>
			...
			<div class='payment_pass' v-if="pay_display">
				<div class='payment_c'>
					...
					<div class="xian_d">
						...
						<div class='forgetPW' @tap='forgetPW()'>忘记密码？</div>
					</div>
		...
		methods:{
			...
			payThree() {
				...
				if (this.useWallte) {
					...
					if (me.wxPayStatue || me.aliPayStatue) {
							var data = {
								module: 'app',
								action: 'order',
								app: 'payment',
				...
				if(this.price2==0){
					...
					uni.request({
						...
						success: (res) => {
							if(res.statusCode==200){
								uni.redirectTo({
									url: "../../pages/pay/payResult?payment_money=" + me.price2 + "&sNo=" + res.data.data.sNo
								})
							}
						},
					})
			...
			_axios(){
				...
				uni.request({
					...
					success:(res) => {
						...
						me.freight = freight;
						if(res.data.products[0]&&res.data.products[0].list[0]){
							me.jp_imgurl = res.data.products[0].list[0].img
							me.jp_name = res.data.products[0].list[0].product_title
							
							let pro = {}
							pro.size=res.data.products[0].list[0].size
							pro.num=res.data.products[0].list[0].num
							me.pro=pro
						}
